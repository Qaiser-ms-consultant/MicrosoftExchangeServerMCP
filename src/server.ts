...
import { registerIndividualMailboxReports } from "./tools/reports-mailbox-individual.js";
import { registerAICoreTools } from "./tools/ai-core.js";
...
async function main() {
  ...
  registerIndividualMailboxReports(server, client.ps);
  registerAICoreTools(server, client.ps);
  ...
  if (transport === "stdio") {
    ...
    console.error(`Exchange MCP server running (stdio) — endpoint=${config.exchange.endpoint} provider=${config.exchange.provider} insecure=${insecure}${insecure ? " [DEV: self-signed allowed]" : ""} | tools=195 (all open)`);
  } else {
