  ...
  server.tool(
    ...
    async ({ identity, days }) => {
      ...
      const [stats, quotaInfo, oof, perms, fwd, rules, cas, health, cert] = await Promise.all([
        ...
        ps.invokeJson(`Get-MailboxPermission -Identity '${id}' | Select-Object User,AccessRights`).then((rows: any[]) => rows.filter((r: any) => !/NT AUTHORITY/i.test(String(r.User ?? "")))).catch(() => []),
        ...
        ps.invokeJson(`Get-InboxRule -Mailbox '${id}' | Select-Object Name,Enabled,ForwardTo,RedirectTo`).catch(() => []),
        ...
        serverFilter
          ? ps.invokeJson(`Get-ServerHealth${serverFilter} | Select-Object HealthSet,AlertValue`).then((rows: any[]) => rows.filter((r: any) => String(r.AlertValue ?? "") !== "Healthy")).catch(() => [])
          : Promise.resolve([]),
        ps.invokeJson(`Get-ExchangeCertificate | Select-Object Subject,NotAfter`).then((rows: any[]) => { const cutoff = Date.now() + 30 * 86400 * 1000; return rows.filter((r: any) => { const t = Date.parse(String(r.NotAfter ?? "")); return !isNaN(t) && t < cutoff; }); }).catch(() => []),
      ]);
