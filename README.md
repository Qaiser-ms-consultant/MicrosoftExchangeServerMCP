...
- **AI-native:** `ai.*` suite (`src/tools/ai-*.ts:1`) — rule-based narrative on live data (Health 82/100, 71× anomaly, 680GB cleanup, etc.), no API key required.
...
| **Search / Mailbox / Groups / Recovery** (21) | `Search-Mailbox`, `Get-MessageTrackingLog`, `New-MoveRequest`, `Get-MailboxFolderStatistics`, `Disable/Connect/Undo-SoftDeleted`, `Restore-RecoverableItems`, `New-MailboxImport/RestoreRequest` | `exchange_search_mailbox`, `exchange_get_message_tracking_log`, `mailbox.get_folder_statistics`, `mailbox.set_quota`, `mailbox.new_move_request`/`get_move_request_status`, `mailbox.add/remove_permission`, `group.new`/`add_member`, `contact.list`, `exchange_disable_mailbox`, `exchange_connect_mailbox`, `exchange_undo_softdeleted_mailbox`, `exchange_get_softdeleted_mailbox`, `exchange_enable_mailbox`, `exchange_restore_recoverable_items`, `exchange_new_mailbox_restore_request`/`get_mailbox_restore_request`, `exchange_new_mailbox_import_request`/`get_mailbox_import_request`/`get_mailbox_import_request_statistics`/`remove_mailbox_import_request` |
...
| **Mailbox (EWS/REST)** (13) — also open | `EWS`, `REST` | `exchange_list_messages`, `exchange_get_message`, `exchange_send_message`, `exchange_reply_message`, `exchange_forward_message`, `exchange_delete_message`, `exchange_move_message`, `exchange_search_messages`, `exchange_list_calendar_events`, `exchange_create_calendar_event`, `exchange_get_availability`, `exchange_list_contacts`, `exchange_list_tasks` (`src/tools/mail-tools.ts:1`, `calendar-tools.ts:1`, `contact-tools.ts:1`) |

### 🤖 AI Suite — 22 tools (`src/tools/ai-*.ts:1`) — rule-based on live data, no API key

| # | Tool | What it does (your spec) |
|---|---|---|
| 16 | `ai.exchange_executive_summary` | Health `82/100` + `Critical: 2 DBs unhealthy, 1 cert 17d` + `Warning: 3 DBs 90d, 27 inactive 1.2TB` + `Recommendation: Move 14 DB04→DB07 + renew EXCH02` |
| 17 | `ai.root_cause_analysis` | Queue `3,452` → investigates SMTP/DNS/connector/CPU/disk → `Outbound queue growth is caused by repeated connection failures to example.com. DNS OK, SMTP timeout` |
| 18 | `ai.anomaly_detection` | `40/day → 2,842 today = 71×` — flags sender/queue/DB/CPU/NDR anomalies |
| 19 | `ai.capacity_forecast` | `DB05 90% in 67d`, `User X 3.2 GB/mo → quota 5mo`, `EXCH03 CPU constrained` |
| 20 | `ai.cleanup_recommendation` | `143 inactive → 74 >180d / 31 disabled AD / 18 departed / 12 shared / 8 system`, `680 GB recoverable` |
| 21 | `ai.security_risk_report` | `HIGH` — `3 external forwarding, 2 abnormal volume, SMTP AUTH 17, anonymous relay, legacy` |
| 22 | `ai.permission_risk_report` | `FullAccess >20, SendAs sensitive, external, former employees` |
| 23 | `ai.compromised_account_detection` | `25–50/day → 1,820/day, 94% external, 02:14 UTC` |
| 24 | `ai.mail_flow_intelligence` | `1.2M (34% vs 7d avg)`, top senders/domains, NDR/delay |
| 25 | `ai.ndr_intelligence` | `1,824 failures → Invalid 842 / Blocked 421 / Unavailable 311 / Policy 182`, `domain.com stale contacts` |
| 26 | `ai.configuration_risk` | `Broad receive connector, anonymous relay, weak TLS, legacy auth, excessive perms` |
| 27 | `ai.migration_advisor` | `87% ready — 1,842 ready / 43 blocked (17 capacity, 11 corrupted…)` for `Exchange 2013→2019` |
| 28 | `ai.migration_prioritization` | `Batch 1 Low-risk 250 → Batch 4 VIP 50` by size/health/importance |
| 29 | `ai.migration_eta` | `327 remaining, 38/h, 8h 35m` (from `Get-MoveRequest`) |
| 30 | `ai.ask_exchange` | Front door — `"Show me mailboxes over 50 GB."` → generates `Get-MailboxStatistics` PowerShell |
| 31 | `ai.comparative_report` | `EXCH01 vs EXCH02`, `DB01 vs DB02`, `This month vs last`, `Prod vs DR` |
| 32 | `ai.change_impact_report` | `Changing Receive Connector X may affect 3 apps` |
| 33 | `ai.what_if_analysis` | `Move 500 DB01→DB05 → 81% capacity, distribute to DB07` |
| 34 | `ai.incident_report` | `Incident #2026-0903`, `Impact ~18,400 delayed, Duration 32m, Recommendation: Add monitoring` |
| 35 | `ai.daily_report` | `🟢 Healthy, 1.28M processed, 37 NDRs, 4 delayed, 2 certs expiring` |
| 36 | `ai.things_you_should_know` | Proactive `3 things: DB03 +42%, cert 19d, NDR +63%` |
| 37 | `ai.management_report` | CTO dashboard — health, availability, mail volume, security, capacity, migration |

### 📊 Reports — 80+ tools (`src/tools/reports-*.ts:1`, `src/tools/admin-reports*.ts:1`)

**Mailbox (35):** `report.mailbox_inventory`, `report.mailbox_distribution`, `report.mailbox_location`, `report.mailbox_size`, `report.largest_mailboxes`, `report.smallest_mailboxes`, `report.mailbox_growth`, `report.fastest_growing_mailboxes`, `report.mailbox_quota`, `report.mailboxes_near_quota`, `report.mailbox_over_quota`, `report.mailbox_type`, `report.shared_mailbox`, `report.room_mailbox`, `report.equipment_mailbox`, `report.arbitration_mailbox`, `report.discovery_mailbox`, `report.archive_mailbox`, `report.mailbox_litigation_hold`, `report.mailbox_retention_policy`, `report.mailbox_inactive`, `report.mailbox_disabled_users`, `report.orphaned_mailboxes`, `report.mailbox_database_distribution`, `report.mailbox_statistics`, `report.mailbox_last_logon`, `report.mailbox_creation_trend`, `report.mailbox_deletion_trend`, `report.mailbox_forwarding`, `report.mailbox_delegation`, `report.mailbox_autoreply` (OOF), `report.mailbox_mobile_access`/`owa_access`/`mapi_access`/`pop_imap`/`ews_usage`

**Database (23):** `report.database_inventory`, `report.database_size`, `report.database_growth`, `report.database_growth_forecast`, `report.database_free_space`, `report.database_whitespace`, `report.database_mailbox_distribution`, `report.database_health`, `report.database_copy_health`, `report.content_index_health`, `report.database_backup_status`, `report.database_restore_readiness`, `report.transaction_log_report`, `report.log_growth_report`, `report.database_io_report`, `report.database_performance`, `report.database_availability`, `report.database_activation_preference`, `report.database_failover_history`, `report.database_maintenance`, `report.database_quota_distribution`, `report.database_utilization`, `report.database_capacity_forecast`

**DAG & Mail Flow (30+):** `report.dag_overview`/`dag_health`/`dag_member_health`/`database_copy_status`/`copy_queue_length`/`replay_queue_length`/`content_index_status`/`active_database_distribution`/`activation_preference`/`dag_network_configuration`/`dag_failover_history`/`ha_readiness` + `report.mail_flow_overview`/`message_volume`/`inbound/outbound/internal_mail`/`top_senders`/`top_recipients`/`top_external_domains`/`message_tracking`/`delivery_status`/`ndr_report`/`ndr_trend`/`queue_report`/`retry_report`/`large_message_report`/`mail_flow_by_connector`/`smtp_traffic`

**Infrastructure (15):** `report.exchange_environment_overview`, `report.exchange_server_inventory`, `report.exchange_version_and_cu`, `report.exchange_build_compliance`, `report.server_role_report`, `report.exchange_topology`, `report.exchange_organization_configuration`, `report.ad_site_exchange_mapping`, `report.exchange_server_os`, `report.exchange_server_hardware`, `report.exchange_virtualization`, `report.exchange_server_uptime`, `report.exchange_service_status`, `report.exchange_dependency`, `report.exchange_infrastructure_summary`

**Other:** `report.generate_*` (health, mailbox size, cert expiry, DB growth, transport queue, hold, OOF, archive, DAG, compliance, full summary) + `report.*` for connectors, security, transport rules, permissions, client access, performance, config, migration, audit, health dashboard — **28 report.* + 10 generate_***
